 ...
 *
 * Composant fourni, repris TEL QUEL (aucune modification de son fonctionnement :
 * carte du monde en base64, points de terre/mer, lettres projetées sur les
 * continents, épingles au clic, glisser-déposer inertiel, zoom à la molette).
 *
 * Seule adaptation, côté export : le preset est typé pour TypeScript strict et
 * `style` est fusionné en dernier pour que l'appelant puisse neutraliser le
 * `minWidth: 1200 / minHeight: 800` du conteneur (sinon, débordement horizontal
 * sur mobile). Aucun effet sur le rendu demandé.
 */
...
const LAND_B64 =
    ...
    "AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA" +
    "AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA" +
    "AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAwAAAAAAAAAAAAAAAAAAAAAAAAABAAAAAA" +
...
export default function GlobeStudy(props: Record<string, unknown>) {
  ...
  // Comportement d'origine intact (preset appliqué en premier, props ensuite).
  const merged = { ...__originkitPresetProps, ...props } as unknown as Props;
  return <__OriginkitBase_GlobeStudy {...merged} />;
}
